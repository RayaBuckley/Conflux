...
Across approximately 1.5 million explored execution traces, no successful
privilege escalation or information-exfiltration violations were observed,
while all tasks already authorised by the underlying access-control system
remained achievable. We additionally report preliminary results from SLED-V,
...
utility for all tasks consistent with the underlying access-control
system.
...
argument values, not control flow. CaMeL enforces developer-defined capability
...
\midrule
CaMeL & Policy/data-flow enforcement & Control flow from trusted user query; untrusted data constrained from modifying it & Developer-defined policies & Capabilities / data-flow provenance \\
Progent & Privilege-policy enforcement & LLM generates initial privilege policy from user task; policy can only narrow without approval & Symbolic/custom policy & Argument/tool metadata \\
PACT & Argument-sensitive enforcement & Argument-level provenance determines if untrusted content influences authority-bearing arguments & Contracts/policies & Argument provenance \\
FORGE & History-sensitive policy enforcement & Observability service maintains execution-context predicates for Datalog enforcement & Datalog policies & Causal/history information \\
...
distinguished by its broader threat model: any principal's information,
including the initiating user's, may arbitrarily determine proposals, and
authority is constrained after influence occurs.}
...
organisational policy. Preliminary SLED-V results, including defect
...
model while preserving utility for ACS-authorised tasks. SLED evaluates implementations under
