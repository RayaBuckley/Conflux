...
## IR-encoded verification evidence tracks

The following evidence tracks are generated by offline scripts and stored
under `research/output/runs/`:

| Track | Script | Description |
|---|---|---|
| Delegation IR | `scripts/generate_delegation_ir_evidence.py` | 11 mutation variants, 11 safety invariants, reference interpreter verdicts |
| Plan IR | `scripts/generate_plan_ir_evidence.py` | Extended plan IR with delegation, monotonic confinement, revocation, liveness |
| Self-composition | `scripts/generate_self_composition_evidence.py` | Symmetry reduction vs. original product IR verdict comparison |
| Refinement | `scripts/generate_refinement_evidence.py` | Assume/guarantee contracts and CEGAR loop on delegation IR variants |
| SLED depth sweep | `scripts/generate_sled_depth_sweep.py` | Native SLED across 4 bound configurations on delegation and planning systems |
| SLED environment sweep | `scripts/generate_sled_environment_sweep.py` | Native SLED across varied principal/action/data-item counts |
| Cross-backend validation | `scripts/generate_backend_cross_validation.py` | Z3 BMC vs. reference interpreter on delegation IR variants |

Key findings:
- All delegation IR mutants are correctly detected as UNSAFE; canonical is SAFE.
- Z3 BMC returns `bounded_safe` for canonical (expected: BMC cannot prove
  exhaustion), while the reference interpreter returns `safe` (exhaustive
  exploration). This is the expected semantic difference, not a bug.
- Planning depth sweep produces `bounded_safe` with 19–67 states across
  bound configurations, confirming non-vacuous exploration.
- Multi-grant delegation IR scales exponentially (2 grants = 25 states,
  3 grants = 125 states).
