...
| AgentDojo establishes Conflux utility or efficacy | Bounded evidence | `research/output/runs/agentdojo-7b-v1/`: six-cell protocol executed with Qwen2.5-7B-Instruct NF4 on RTX 4060; all six cells completed (native_security=True for all cells; utility=False for all cells — the 7B model calls `search_emails` correctly but passes an unsupported `sender` argument that the mediator rejects); `research/output/runs/agentdojo-1b5-nf4-v1/`: six cells completed with 1.5B model (native_security=True, utility=False — model too small for multi-turn tool use); `research/output/runs/agentdojo-3b-v1/`: six cells model_failed (3B model crashes on second turn after receiving tool results); the pipeline runs end-to-end including attack injection, ITES mediation, and tool filtering; security metric corrected: `native_security=True` means injection was prevented (previously inverted) |
| Open-ended planning improves utility | Bounded evidence | `research/output/runs/planning-pilot-7b-v1/`: eight-cell pilot completed with Qwen2.5-7B-Instruct NF4; 7/8 cells `utility_completed=True` (only `blocked-action-recovery:dynamic_code` failed as `modeled_program_failed`); `research/output/runs/planning-pilot-1b5-v1/`: eight-cell pilot completed with Qwen2.5-1.5B-Instruct NF4; all cells `complete=True`; model is too small for high utility but the planning pipeline executes end-to-end |
