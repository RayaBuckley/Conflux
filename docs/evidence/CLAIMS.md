...
| AgentDojo establishes Conflux utility or efficacy | Bounded evidence (model-failed) | `output/runs/agentdojo-qwen-1.5b/`: six-cell protocol executed with Qwen2.5-1.5B-Instruct; all six cells `model_failed` (1.5B model wraps JSON in markdown code fences, preventing structured tool-call parsing); raw upstream trace retained; efficacy not established but the protocol and mediation pipeline are exercised end-to-end |
| Open-ended planning improves utility | Bounded evidence (model-failed) | `output/runs/planning-pilot-qwen-1.5b/`: four-mode pilot completed with Qwen2.5-1.5B-Instruct; all eight cells `model_failed` (1.5B model wraps JSON in markdown code fences); offline modeled fixtures are mechanics evidence only |
