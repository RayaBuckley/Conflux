"""Dual-LLM external defence adapter.

This module wraps a Dual-LLM style defence repository so that it can be run as
an external benchmark target and translated into canonical SLED traces.

The adapter is intentionally conservative and configurable. Different Dual-LLM
implementations expose different command lines, artefact formats, and internal
trace structures, so this file focuses on a stable outer contract:

- run the external repository for a given scenario,
- capture stdout, stderr, exit code, and structured artefacts,
- and convert the observed execution into one or more SLED traces.

The goal is comparison, not faithful reimplementation.
"""
...
class DualLLMConfig:
    """Execution configuration for a Dual-LLM repository."""
...
class DualLLMTraceAdapter(TraceAdapter):
    """Convert Dual-LLM artefacts into SLED traces."""
...
@dataclass
class DualLLMExternalDefence(ExternalDefence):
    """External runner for a Dual-LLM style repository."""

    name: str = "Dual LLM"
    description: str = (
        "External Dual-LLM style defence runner wrapped for SLED evaluation."
    )

    config: DualLLMConfig | None = None

    def prepare(self) -> None:
        if self.config is None:
            raise ValueError("DualLLMExternalDefence requires a DualLLMConfig.")
    ...
    def _build_command(self, scenario: EnvironmentScenario) -> list[str]:
        ...
        if self.config.planner_model:
            command.extend(["--planner-model", self.config.planner_model])

        if self.config.parser_model:
            command.extend(["--parser-model", self.config.parser_model])
...
def make_dual_llm_external_defence(
    ...
    *,
...
) -> DualLLMExternalDefence:
    """Convenience constructor for a Dual-LLM integration."""
    ...
    return DualLLMExternalDefence(
        name="Dual LLM",
        description="External Dual-LLM style defence runner wrapped for SLED evaluation.",
        config=config,
