...
and value bindings. Generated code is treated as input to a mediated capability
envelope rather than as trusted control flow. Every grounded effect is
re-authorised at execution time. Security is a hard selection constraint;
...
\section{Results}
\textbf{Repository validation:} \emph{retained final-revision result pending}.\\
...
The immediate priority is reproducible evidence: run credentialed real-model
and pinned AgentDojo comparisons, retain native and Conflux metrics, and
generate paper tables solely from those result records. The next engineering
priority is differential validation against a production policy decision point
and an agent-framework boundary without moving benchmark behavior into ITES.

The next security-model extensions are scoped attenuating delegation,
role-sensitive argument effects, and origin-bound persistent-memory authority.
They remain denied or conservative until their semantics, revocation behavior,
and counterexample properties are specified. Formal work should then extend
...
slice, authenticated bounded planning, and native plus solver-facing
verification foundations. The remaining research contribution is stronger
evidence and carefully specified extensions: real-model, external-benchmark,
policy-integration and formal artefacts whose claims remain tied to explicit
assumptions, models, and bounds.
